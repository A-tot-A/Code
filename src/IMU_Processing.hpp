...
/// curvature不是英语的翻译的曲率的意思，这里是时间的意思
...
class ImuProcess
{
 public:
  ...
  V3D cov_bias_gyr;
  V3D cov_bias_acc;
  double first_lidar_time; // 第一针雷达的时间，lidar_beg_time
 ...
 private:
  ...
  vector<Pose6D> IMUpose;// 每个IMU数据前向传播计算的先验知识
...
/**
 * 在UndistortPcl函数中不但有IMU的前向信息，还有激光雷达去畸变的问题，这一节我们围绕着IMU的正向传播展开，代码中通过迭代的形式完成了IMU数据的更新，并将acc和gyro的数据传入到ESKF中，
 */
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  const double &pcl_beg_time = meas.lidar_beg_time;//pcl开始的时间戳，雷达开始的时间戳
  const double &pcl_end_time = meas.lidar_end_time;// 雷达结束的时间戳
    ...
    for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    /// dt是时间变化 Q是协方差矩阵，in是输入
    kf_state.predict(dt, Q, in);

    /* save the poses at each IMU measurements */
      // 保存IMU预测过程的状态,当前的状态的先验概率
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    {
        //在上面IMU_init()基础上乘上缩放系数，pow是次方的意思
