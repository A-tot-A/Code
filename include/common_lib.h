...
typedef pcl::PointXYZINormal PointType;// 点云数据的类型
typedef pcl::PointCloud<PointType> PointCloudXYZI; // 点云的全部信息
...
bool esti_plane(Matrix<T, 4, 1> &pca_result, const PointVector &point, const T &threshold)
{
    ...
    T n = normvec.norm();// n是一个数字，.norm（）返回的是一个，normalize()返回是就是把自身的各元素除以它的范数
